...
COORD coord={0,0};
COORD red_ty={4,10+TOP};
COORD red_st={5,13+TOP};
COORD blue_ty={48,10+TOP};
COORD blue_st={47,13+TOP};
void map_refresh()
{
    setColor(4,0);
    COORD pre_red_coord={2*red.pre_col+LEFT,red.pre_row+TOP};
    COORD red_coord={2*red.col+LEFT,red.row+TOP};
    ...
    setColor(1,0);
    COORD pre_blue_coord={2*blue.pre_col+LEFT,blue.pre_row+TOP};
    COORD blue_coord={2*blue.col+LEFT,blue.row+TOP};
...
//刷新 blue的子弹
COORD blue_0={46,10+TOP};
COORD blue_1={46,8+TOP};
COORD blue_2={46,6+TOP};
COORD blue_3={46,4+TOP};
bullet_refresh_blue()
{
    for(int i=1;i<BUL;i++)
...
void bullet_refresh()
{
    ...
    for(int i=bul.size()-1;i>=0;i--)
    {
        ...
        {
            pre = {2*bul[i].pre_col+LEFT,bul[i].pre_row+TOP};
            SetConsoleCursorPosition(hOutput,pre);
            printf("\b ");
        }
        now = {2*bul[i].col+LEFT,bul[i].row+TOP};
...
{
    COORD pos = {2*b.pre_col+LEFT,b.pre_row+TOP};
