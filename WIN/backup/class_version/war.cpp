...
bool die_p()
{
    ...
    {
        sound_dead_p();
        log_0("    P2赢了，再玩一次吗？(Y/N)");
        return end();
    }
    else if(blue.digit == 0)
    {
        sound_dead_p();
        log_0("    P1赢了，再玩一次吗？(Y/N)");
        return end();
    }
    else
    {
...
player::player(string pos,string c)
{
    if(pos == "left")
    {
        bul_pos_0={6,10+TOP};
    ...
    {
        bul_pos_0={2*COL+16,10+TOP};
        bul_pos_1={2*COL+16,8+TOP};
        bul_pos_2={2*COL+16,6+TOP};
        bul_pos_3={2*COL+16,4+TOP};
    }
...
player::bullet_refresh()
{
    for(int i=1;i<BUL;i++)
        bullet[i-1] = bullet[i];
    bullet[BUL-1] = rand()%9+1;

    if(mycolor == "red") color_red();
    else if(mycolor == "blue") color_blue();
    else color_normal();
    SetConsoleCursorPosition(hOutput,bul_pos_0);
    printf("\b%d",bullet[0]);
    SetConsoleCursorPosition(hOutput,bul_pos_1);
    printf("\b%d",bullet[1]);
    SetConsoleCursorPosition(hOutput,bul_pos_2);
    printf("\b%d",bullet[2]);
    SetConsoleCursorPosition(hOutput,bul_pos_3);
    printf("\b%d",bullet[3]);
    color_back();
